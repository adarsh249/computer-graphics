...
Matrix4x4 parse_transformation(const json &node) {
    ...
    for (auto it = transform_it->begin(); it != transform_it->end(); it++) {
        if (auto scale_it = it->find("scale"); scale_it != it->end()) {
            ...
            rotateM(0, 0) = axis.x*axis.x+((1-(axis.x*axis.x))*cos(angle));
            rotateM(1, 0) = axis.x*axis.y*(1-cos(angle))+(axis.z*sin(angle));
            rotateM(2, 0) = axis.x*axis.z*(1-cos(angle))-(axis.y*sin(angle));
            rotateM(0, 1) = axis.y*axis.x*(1-cos(angle))-(axis.z*sin(angle));
            rotateM(1, 1) = axis.y*axis.y+((1-(axis.y*axis.y))*cos(angle));
            rotateM(2, 1) = axis.y*axis.z*(1-cos(angle))+(axis.x*sin(angle));
            rotateM(0, 2) = axis.z*axis.x*(1-cos(angle))+(axis.y*sin(angle));
            rotateM(1, 2) = axis.z*axis.y*(1-cos(angle))-(axis.x*sin(angle));
            rotateM(2, 2) = axis.z*axis.z+(1-axis.z*axis.z)*cos(angle);
            rotateM(3, 3) = 1.0;
            F = rotateM * F;
            ...
            translateM(2, 2) = 1;
            translateM(3, 3) = 1;
            translateM(0, 3) = translate.x;
            translateM(1, 3) = translate.y;
            translateM(2, 3) = translate.z;
            F = translateM * F;
            ...
            Vector3 uPrime = cross(r, d);
            Matrix4x4 lookAtM;
            lookAtM(0, 0) = r.x;
            lookAtM(1, 0) = r.y;
            lookAtM(2, 0) = r.z;
            lookAtM(0, 1) = uPrime.x;
            lookAtM(1, 1) = uPrime.y;
            lookAtM(2, 1) = uPrime.z;
            lookAtM(0, 2) = -d.x;
            lookAtM(1, 2) = -d.y;
            lookAtM(2, 2) = -d.z;
            lookAtM(0, 3) = position.x;
            lookAtM(1, 3) = position.y;
            lookAtM(2, 3) = position.z;
            lookAtM(3, 3) = 1.0;
            F = lookAtM * F;
        }
    }
