...
Matrix3x3 parse_transformation(const json &node) {
    ...
    for (auto it = transform_it->begin(); it != transform_it->end(); it++) {
        if (auto scale_it = it->find("scale"); scale_it != it->end()) {
            ...
            Matrix3x3 rotateM;
            rotateM(0, 0) = cos(angle * M_PI/180.0);
            rotateM(0, 1) = -sin(angle* M_PI/180.0);
            rotateM(1, 0) = sin(angle* M_PI/180.0);
            rotateM(1, 1) = cos(angle* M_PI/180.0);
            rotateM(2, 2) = 1;
            F = rotateM * F;
            ...
            translateM(1, 1) = 1;
            translateM(2, 2) = 1;
            translateM(0, 2) = translate.x;
            translateM(1, 2) = translate.y;
            F = translateM * F;
